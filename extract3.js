...
const baseUrl = "http://35.200.185.69:8000/v3/autocomplete";
const results = new Set();
const checkedQueries = new Set();
const zeroResultQueries = new Set(); // Track queries that returned 0 results
const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const letters = "abcdefghijklmnopqrstuvwxyz0123456789 -.".split("");

// Function to log searched queries
function logSearchedQuery(query, count) {
  fs.appendFileSync("searched_queries_v3.txt", `${query},${count}\n`, "utf8");
}

// Function to fetch suggestions
async function fetchSuggestions(query) {
  if (checkedQueries.has(query) || zeroResultQueries.has(query)) return []; // Skip already searched or zero result queries
...
async function start() {
  ...
  fs.writeFileSync("names_v3.txt", Array.from(results).join("\n"), "utf8");
  console.log(`Total names extracted (V3): ${results.size}`);
}
